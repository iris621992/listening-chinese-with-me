...
function build() {
  ...
  for (const file of files) {
    ...
    const top = `<article class="lesson-hero lesson-shell card"><div><p class="badge">${meta.hsk}</p><h1>${meta.title}</h1><p class="lesson-summary">${meta.summary}</p></div><section class="lesson-overview" aria-label="${SECTION_LABELS.video_description}"><h2 data-i18n="about_lesson">${SECTION_LABELS.video_description}</h2>${renderSectionBody(sections.video_description)}</section></article>${studyPlayer}<section class="study-intro lesson-shell card"><h2 data-i18n="study_area">Study Area</h2></section>`;
