...
const LEARNING_LANGUAGE_CHANNELS = [
  { id: 'chinese', flagPath: `${SITE_BASE}/assets/flags/china.svg`, code: 'ZH', label: 'Chinese', enabled: true }
];
const UI_TEXT = {
  en: {
    home: 'Home',
...
function renderLearningSidebar({ lessonCounts = {}, activeLanguage = 'chinese' } = {}) {
  ...
  const languageItems = availableLearningLanguages
    .map((language) => {
      ...
      const flagMarkup = `<img src="${language.flagPath}" alt="${language.label}" class="language-flag-img" loading="lazy" onerror="this.hidden=true;this.nextElementSibling.hidden=false;"><span class="language-flag-fallback" hidden>${language.code}</span>`;
      if (language.id === activeLanguage) classes.push('active');
      return `<span class="${classes.join(' ')}" data-sidebar-expand-on-activate aria-label="${language.label}"><span class="learning-language-flag" aria-hidden="true">${flagMarkup}</span><span class="learning-language-code">${language.code}</span></span>`;
    })
    .join('');
  const navItems = [
...
function copyAssets() {
  ...
  ensure(path.join(distDir, 'assets/flags'));
  fs.copyFileSync(path.join(root, 'src/assets/css/style.css'), path.join(distDir, 'assets/css/style.css'));
  fs.copyFileSync(path.join(root, 'src/assets/flags/china.svg'), path.join(distDir, 'assets/flags/china.svg'));
}
