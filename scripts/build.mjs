...
function renderStudyPlayer({ meta, sections, youtubeEmbed, youtubeWatchButton }) {
  ...
  const summaryBody = renderSectionBody(sections.video_description || '');
...
function build() {
  ...
  for (const file of files) {
    ...
    const top = `<article class="lesson-hero lesson-shell card"><p class="badge">${meta.hsk}</p><div><h1>${meta.title}</h1><p class="lesson-summary">${meta.summary}</p></div></article>${studyPlayer}<section class="study-intro lesson-shell card"><h2 data-i18n="study_area">Study Area</h2></section>`;
