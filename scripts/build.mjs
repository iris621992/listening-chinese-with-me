...
const UI_TEXT = {
  en: {
    ...
    layer2_desc: 'Choose English or Tiếng Việt for the website UI.',
...
function toHtmlSections(md) {
  ...
  const fixedSections = STUDY_SECTIONS.map((key) => `<section class="card lesson-section" data-section="${key}"><h2 data-i18n="${key}">${SECTION_LABELS[key]}</h2>${renderSectionBody(sections[key] || '<p class="missing">(Content missing)</p>')}</section>`).join('');
