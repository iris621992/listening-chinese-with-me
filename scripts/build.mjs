...
function renderStudyPlayer({ meta, sections, youtubeEmbed, youtubeWatchButton }) {
  ...
  return `<section class="study-player-layout card" aria-label="Video study player"><div class="study-mode-tabs" role="tablist" aria-label="Study modes"><button type="button" class="study-mode-tab is-active" data-i18n="shadowing">Shadowing</button><button type="button" class="study-mode-tab" data-i18n="pronunciation">Pronunciation</button><button type="button" class="study-mode-tab" data-i18n="dictation">Dictation</button><button type="button" class="study-mode-tab" data-i18n="summary_tab">Summary</button></div><div class="study-player-grid"><div class="study-video-column"><section class="lesson-video" aria-label="Lesson video">${videoBlock}</section></div><aside class="transcript-panel" aria-label="Transcript"><div class="transcript-panel-header"><h2 data-i18n="transcript">Transcript</h2><span>${meta.hsk}</span></div><ol class="transcript-lines">${renderTranscriptLines(sections.chinese || '')}</ol></aside></div></section>`;
}

function toHtmlSections(md) {
  const sections = parseSections(md);
  const fixedSections = STUDY_SECTIONS.map((key) => `<section id="content-${key}" class="card lesson-section" data-section="${key}"><h2 data-i18n="${key}">${SECTION_LABELS[key]}</h2>${renderSectionBody(sections[key] || '<p class="missing">(Content missing)</p>')}</section>`).join('');
  ...
  const translationSections = ['english', 'vietnamese'].map((key) => `<section class="card lesson-section translation-section" data-translation="${key}"><h2 data-i18n="translation">${SECTION_LABELS[key]}</h2>${renderSectionBody(sections[key] || '<p class="missing">(Content missing)</p>')}</section>`).join('');
  ...
  return `${contentTabs}${fixedSections}${translationSelector}${translationSections}`;
}
